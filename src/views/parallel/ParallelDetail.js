...
ParallelDetail.propTypes = {
  sx: PropTypes.object,
};

export default function ParallelDetail(props) {
  ...
  const submit = async () => {
    ...
    if (actionCode === "CREATE") {
      await axios.post(endpoint.parallel.root, params);
    } else {
      params = { ...params, id: state.id };
      await axios.put(endpoint.parallel.root, params);
    }
