...
JurusanDetail.propTypes = {
  sx: PropTypes.object,
};

export default function JurusanDetail(props) {
  ...
  const submit = async () => {
    ...
    if (actionCode === "CREATE") {
      await axios.post(endpoint.jurusan.root, params);
    } else {
      params = { ...params, id: state.id };
      await axios.put(endpoint.jurusan.root, params);
    }
