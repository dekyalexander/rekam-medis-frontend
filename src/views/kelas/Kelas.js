...
import KelasListHead from './KelasListHead';
import KelasListToolbar from './KelasListToolbar';
...
export default function Kelas() {
  ...
  const getData = async (newPage, newRowsPerPage) => {
    ...
    if(user.user_type && user.user_type.code==='STUDENT'){
      kelas_id = user.kelas?user.kelas.id:null
    }
    let params = {
    ...
    const response = await axios.get(endpoint.kelas.root, { params: params });
  ...
  const syncKelas = async(jenjang_code) => {
    startLoading(jenjang_code)
    const params = {
      jenjang_code: jenjang_code
    };
    const response = await axios.post(endpoint.kelas.sync, params)
  ...
  const handleDelete = async () => {
    ...
    const response = await axios.delete(endpoint.kelas.root, {
      data: params
    });
    if (response) {
