...
const TABLE_HEAD = [
  { id: "drug_unit", label: "Satuan Obat", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_user) =>
        _user.drug_unit.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function Drug() {
  ...
  const [orderBy, setOrderBy] = useState("drug_unit");
  ...
  const handleDelete = async () => {
    ...
    const response = await axios.delete(endpoint.drug_unit.delete, {
      data: params,
    });
    if (response) {
            ...
            <Table>
              ...
              <TableBody>
                {filteredData
                  ...
                  .map((row) => {
                    const { id, drug_unit, created_at } = row;
