...
import UnitListHead from "./UnitListHead";
...
const TABLE_HEAD = [
  ...
  { id: "head_role_id", label: "Head Role", alignRight: false },
];
...
export default function Unit() {
  ...
  const getData = async () => {
    const response = await axios.get(endpoint.unit.root);
  ...
  const handleDelete = async () => {
    ...
    const response = await axios.delete(endpoint.unit.root, {
      data: params,
    });
    if (response) {
            ...
            <Table>
              ...
              <TableBody>
                {filteredData
                  ...
                  .map((row) => {
                    const { id, name, head_role_id, role } = row;
