...
export default function DrugDetail(props) {
  ...
  const [state, setstate] = useState({
    ...
    drug_id: "",
    location_id: "",
    drugexpired: { date_expired: new Date() },
    stocks: { qty: "" },
    locationdrug: { location_name: "" },
    drug_expired_id: "",
  });
  ...
  const ValidationForm = Yup.object().shape({
    ...
    location_name: Yup.string().required("Lokasi Obat Tidak Boleh Kosong"),
  ...
  const handleChange = (fieldName, eventValue) => {
    ...
    const header_detail = ["qty", "location_name", "date_expired"];
  ...
  let params = {
    ...
    description: state.description,
    location_name: state.locationdrug.location_name,
    // drug_id: state.drug_id,
    // location_id: state.location_id,
    // drug_expired_id: state.drug_expired_id,
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      if (actionCode === "CREATE") {
        ...
        let respon = await axios
          .put(endpoint.drug.update, values)
          .catch(function (error) {
          ...
          <Grid container style={{ marginBottom: 16 }}>
            ...
            >
              ...
              <Select
                ...
                value={state.locationdrug.location_name || ""}
                onChange={(e) => handleChange("location_name", e)}
