...
export default function DiagnosisDetail(props) {
  ...
  const submit = async () => {
    ...
    if (actionCode === "CREATE") {
      await axios.post(endpoint.diagnosis_mcu.create, params);
    } else {
      params = { ...params, id: state.id };
      await axios.put(endpoint.diagnosis_mcu.update, params);
    }
...
*/
  ...
  //     if (actionCode === "CREATE") {
  //       await axios.post(endpoint.diagnosis_mcu.create, values);
  //     } else {
  //       await axios.put(endpoint.diagnosis_mcu.update, values);
  //     }
  //     setSubmitting(false);
  //     submitSuccess("saving data success");
  //     getData();
    ...
    onSubmit: async (values, { setSubmitting }) => {
      if (actionCode === "CREATE") {
        let respon = await axios
          .post(endpoint.diagnosis_mcu.create, values)
        ...
        let respon = await axios
          .put(endpoint.diagnosis_mcu.update, values)
          .catch(function (error) {
