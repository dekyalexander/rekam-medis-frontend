...
const TABLE_HEAD = [
  { id: "uks_name", label: "Lokasi Obat", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_user) =>
        _user.uks_name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function Drug() {
  ...
  const handleDelete = async () => {
    ...
    const response = await axios.delete(endpoint.drug_location.delete, {
      data: params,
    });
    if (response) {
            ...
            <Table>
              ...
              <TableBody>
                {filteredData
                  ...
                  .map((row) => {
                    const { id, uks_name, created_at } = row;
