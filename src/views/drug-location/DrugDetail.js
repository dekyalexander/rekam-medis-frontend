...
export default function DrugDetail(props) {
  ...
  const ValidationForm = Yup.object().shape({
    uks_name: Yup.string()
      ...
      .required("Lokasi Obat Tidak Boleh Kosong"),
  });
  ...
  const submit = async () => {
    ...
    if (actionCode === "CREATE") {
      await axios.post(endpoint.drug_location.create, params);
    } else {
      params = { ...params, id: state.id };
      await axios.put(endpoint.drug_location.update, params);
    }
...
*/
  ...
  //     if (actionCode === "CREATE") {
  //       await axios.post(endpoint.drug_location.create, values);
  //     } else {
  //       await axios.put(endpoint.drug_location.update, values);
  //     }
  //     setSubmitting(false);
  //     submitSuccess("saving data success");
  //     getData();
    ...
    onSubmit: async (values, { setSubmitting }) => {
      if (actionCode === "CREATE") {
        let respon = await axios
          .post(endpoint.drug_location.create, values)
        ...
        let respon = await axios
          .put(endpoint.drug_location.update, values)
          .catch(function (error) {
        ...
        <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
          <Grid container style={{ marginBottom: 16 }}>
            <TextField
              ...
              value={state.uks_name}
              onChange={(e) => handleChange("uks_name", e)}
              fullWidth
              autoComplete="on"
              type="text"
              label="Lokasi Obat"
              error={Boolean(touched.uks_name && errors.uks_name)}
              helperText={touched.uks_name && errors.uks_name}
              sx={{ mb: 5 }}
            />
          </Grid>
