...
const TABLE_HEAD = [
  { id: "drug_type", label: "Tipe Obat", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_user) =>
        _user.drug_type.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function Drug() {
  ...
  const [orderBy, setOrderBy] = useState("drug_type");
  ...
  const handleDelete = async () => {
    ...
    const response = await axios.delete(endpoint.drug_type.delete, {
      data: params,
    });
    if (response) {
            ...
            <Table>
              ...
              <TableBody>
                {filteredData
                  ...
                  .map((row) => {
                    const { id, drug_type, created_at } = row;
